...
double ElementsDivision(Vector myvector){
    try{
        double result = myvector.GetV()[0] / myvector.GetV()[1];
        return isinf(result) ? throw invalid_argument("Cannot divide by 0") : result;
    }
    catch(invalid_argument myexception){
...
int main(int argc, char* argv[]){
    ...
    string filename = "myvector.txt";

    Vector x(LineCounter(filename));
